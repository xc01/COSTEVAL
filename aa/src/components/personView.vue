<template>
  <div>
    <el-page-header @back="$emit('back')" content="查看项目相关人员">
    </el-page-header>
    <el-input
      placeholder="请输入用户名称"
      v-model="keyword"
    ...
    >
      <el-button slot="append" :icon="ElIconSearch" @click="search"></el-button>
    </el-input>
    <ul>
      <li v-for="person in personList" :key="person.uid">
        <span>{{ person.username }}</span>
        <el-button @click="BeginEdit(person.name, person.uid, person.priority)"
          >修改权限</el-button
        >
      </li>
    </ul>
...
export default {
  ...
  methods: {
    ...
    BeginEdit: function (name, uid, prio) {
      // 开始修改
    ...
    submit: function () {
      ...
      if (this.selectedPriority < 0) {
        ...
        for (let i = 0; i < this.personList.length; i++) {
          if (this.personList[i].uid == this.selectedUid) {
            // 找到了需要移除的人员
