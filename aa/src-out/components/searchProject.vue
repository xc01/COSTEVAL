...
export default {
  ...
  methods: {
    addNewProject: function () {
      ...
      this.$prompt('请输入项目名', '创建新项目', {
        ...
        .then(({ value }) => {
          ...
          $emit(this, 'addProjectSuccess', new_project) // 触发事件 addProjectSuccess, 以便父组件添加到列表
    ...
    addToProject: function (item) {
      // test
      $emit(this, 'participateNewProject', item) // 触发事件 participateNewProject , 使主页面更新
  ...
  emits: ['addProjectSuccess', 'participateNewProject'],
}
</script>
